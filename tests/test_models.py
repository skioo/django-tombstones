...
from .models import Person, Vehicle, Car, VicePrincipal
...
class SingleModelTest(TestCase):
    ...
    def test_it_should_not_introduce_extra_queries(self):
...
class OneToManyRelationshipTest(TestCase):
    ...
    def test_it_cannot_hide_objects_on_the_one_side_of_the_relation(self):
        ...
        that object stays navigable even though it was soft-deleted.
        """
        bob = Person.objects.get(name='bob')
        bob.soft_delete()
...
class OneToOneRelationshipTest(TestCase):
    """
    When two objects are in a one-to-one relationship, then soft-deleting one has no impact on the other.
    """
